...
fomc_release_dates = list(filter(lambda x: x >= datetime(year=2022, month=4, day=1), fomc_release_dates))
...
for date in fomc_release_dates:
    df = pd.read_csv(f'data/market_ticks/40_min/{date.month:02}_{date.day:02}_{str(date.year)[2:]}.csv')
